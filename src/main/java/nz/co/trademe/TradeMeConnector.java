...
import ch.ike.trademe_scanner.TradeMeScannerPersistenceConnection;
...
public class TradeMeConnector {
	...
	public void checkAuthorisation() {
		...
		try {	
			Entry<String, String> accessTokenData = persistence.getAccessToken(connection);
			if ((accessTokenString != null) && (accessSecretString != null)) {
				...
				persistence.setAccessToken(accessToken.getToken(), accessToken.getSecret(), connection);
	...
	public void deauthoriseUser() {
		TradeMeScannerPersistenceConnection connection = persistence.getConnection();
		try {	
			persistence.deleteAccessToken(connection);
		} finally {
			connection.close();
		}
